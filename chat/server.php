...
//--------------------------------------------------------------------------------------------------------------------------------	
			...
			//verification
			$sql = BaseDonnee::execQuery($bdd, "SELECT * From Users WHERE Username = '$user_name'");
			if(empty($sql)){ //Un user non inscrit a réussi a envoyer un msg
				$response = mask(json_encode(array('type'=>'system', 'message'=>"Register and/connect please!")));
				@socket_write($changed_socket,$response,strlen($response));
				break 2;
			}else{
				$maxlifetime = ini_get("session.gc_maxlifetime"); // durée d'une session
				$lastseen = strtotime($sql[0]["LastTimeSeen"]); // derniere fois que l'user a chargé une page
				$now = time(); // temps actuel
